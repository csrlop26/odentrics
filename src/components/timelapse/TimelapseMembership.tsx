...
export default function TimelapseMembership({ isActive = true, isAutoplay = false, onComplete, onOpenBooking }: TimelapseMembershipProps) {
  const step = useTimelapse(7, isActive, isAutoplay, onComplete);
  ...
  const getCodeSnippet = (s: number) => {
    switch (s) {
      ...
      case 2: return "/* Inject Decorative Blurs */\n<div className=\"blur-circle right\" />\n<div className=\"blur-circle left\" />";
      case 3: return "/* Typography */\nfont-family: 'Inter', sans-serif;\ntext-align: center;";
      case 4: return "/* Load Membership Plans */\nconst plans = getPlans();\n<Grid layout=\"2-cols\" />";
      case 5: return "/* Render Plan Cards */\nplans.map(p => <PlanCard />)\n// Applying slate theme to premium...";
      case 6: return "/* Add Savings Calculator */\n<CalculatorWidget />\nbackdrop-filter: blur(10px);";
      case 7: return "/* Section Finished */";
      default: return "";
    }
  };

  return (
    <section id="circulo" className="py-24 relative overflow-hidden transition-colors duration-1000" style={{ backgroundColor: step >= 1 ? '#FDFBF7' : '#ffffff' }}>
      ...
      <AnimatePresence>
        {step >= 2 && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 0.4 }} transition={{ duration: 1 }} className="absolute right-0 top-1/4 w-96 h-96 bg-slate-100 rounded-full filter blur-3xl pointer-events-none" />
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 0.3 }} transition={{ duration: 1 }} className="absolute left-10 bottom-10 w-80 h-80 bg-emerald-50 rounded-full filter blur-3xl pointer-events-none" />
          </>
        )}
      </AnimatePresence>
      ...
      <div className="max-w-[1440px] mx-auto px-6 md:px-[80px] relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl mx-auto mb-16">
          <AnimatePresence>
            {step >= 5 && MEMBERSHIP_PLANS.map((plan, idx) => {
                ...
                >
                  ...
                  <div className="mt-8 pt-6">
                    {isSlateCard ? (
                      ...
                      <button onClick={() => onOpenBooking && onOpenBooking(plan.id)} className="w-full py-4 text-xs font-bold uppercase tracking-widest rounded-full bg-[#2C3E48] text-white hover:bg-[#1A252C] transition-all active:scale-[0.98] shadow-md flex items-center justify-center gap-2">
        ...
        <AnimatePresence>
          {step >= 6 && (
            <motion.div 
              ...
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.6 }}
              className="max-w-4xl mx-auto bg-white/70 backdrop-blur-md rounded-3xl border border-[#deeaf3] p-8 shadow-sm"
            >
      ...
      {step < 7 && !isAutoplay && (
        <div className="fixed top-2 left-2 text-[10px] font-mono text-gray-400 opacity-50 select-none pointer-events-none z-50">
          TIMELAPSE MODE [MEMBERSHIP] - STEP {step}/7
        </div>
      )}
    </section>
  );
